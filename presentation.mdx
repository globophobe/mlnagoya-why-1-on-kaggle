...
## レッスン1と2は、簡単に転送学習ト用のResNetを使用してCNNを作成方法を説明します。
...
# Because the 1.0 scores had the same data augmentation parameters.
## 1.0のスコアのデータ増強のパラメータは同じでしたから。
---
...
# Except for one parameter, they are all default parameters.
...
# The solution for 1.0 was to correct imbalanced classes.
## 1.0の解決策は、不均衡なクラスを修正することでした。

---

# Anway, wait and see what happens on the private leaderboard when the contest ends.
## Kaggleコンテスト終わってから、プライベートリーダーボードはどうなるかなを楽しみに。

---

## To close, a comment to "why 1.0?", "Perhaps it's not such a prickly problem."
## 「なぜ1.0」に対応のコメントは「おそらく空中サボテンの同定の問題はそなに難しくない。」
